...
6. Delegate approved integration to `knowledge-integrator` with operation type `ingest` (OKF frontmatter + Markdown links only).
7. Delegate the resulting content diff to `wiki-verifier` in `mutation` mode (okf-core + second-brain-governed).
