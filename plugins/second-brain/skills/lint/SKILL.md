...
- `incremental` when `$ARGUMENTS` includes `incremental` or the caller is maintain in incremental mode with a non-empty baseline: compute candidates via `git diff --name-only <last_successful_commit>`, keep only `wiki/**/*.md` paths (plus one-hop wiki links you discover), and pass that filtered list to the curator.
- Prefer `incremental` for routine maintain; prefer `full` for an explicit `/second-brain:lint` without args.

When `$ARGUMENTS` includes `verify` or `strict` (including `/second-brain:lint verify` from maintain), also ask `wiki-verifier` in `audit` mode to sanity-check the curator report. Otherwise do not invoke the verifier.
