...
Delegate changes to `knowledge-integrator` with operation type `repair` and an approved change package derived from the lint findings (include the intended `knowledge/log.md` entry in that package). Require `knowledge-verifier` (`mutation` mode) to pass on the full package. Do not mutate files after PASS. Retain unresolved findings in the report/summary.
